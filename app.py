...
st.set_page_config(
    page_title="ReBrew NeuroFlow – Subscription Intelligence",
    layout="wide",
    page_icon="☕",
)
...
def show_executive_summary(df):
    ...
    <div style='padding:24px 28px;border-radius:24px;
        background:linear-gradient(135deg,#4c6fff 0%,#a855f7 40%,#ec4899 100%);
        color:white;box-shadow:0 18px 45px rgba(15,23,42,0.55);margin-bottom:18px;'>
        <h1 style='margin-bottom:4px;'>☕ ReBrew NeuroFlow – AI Marketing Co‑Pilot</h1>
        <p style='font-size:0.96rem;opacity:0.92;'>
            ChatGPT‑style cockpit that turns survey data into subscription intelligence for the ReBrew team.
        </p>
    ...
    with c1:
        ...
        st.write(
            ...
            - 🧵 **Customer Storytelling** – Translate raw survey rows into narratives & personas.  
        ...
        with st.expander("📌 Business Impact (click to open)"):
            st.write(
                ...
                - Build an always‑on **lead‑scoring engine** for remarketing & email flows.  
                - Design **personalised bundles** for 'Maybe' segment to push them into 'Yes'.  
                - Provide **investor‑ready visuals** for strategy reviews and pitches.  
                """
            )
    with c2:
        st.subheader("👨‍👩‍👧‍👦 NeuroFlow Squad")
        st.write("Core team behind this AI‑powered cockpit:")
...
def show_ml_lab(df):
    st.subheader("🧪 AI Lab – Classification, Clustering, Rules & Regression")
...
def show_prediction_studio(df):
    st.subheader("🔮 Prediction Studio – Score a New Customer")
    if "pipelines" not in st.session_state:
        st.info("First run a classification experiment in the AI Lab tab.")
        return
...
def main():
    ...
    with st.sidebar:
        st.markdown("## ☕ ReBrew NeuroFlow")
        st.write("AI‑Powered Subscription Intelligence")
        st.markdown("---")
        nav = st.radio(
            "Navigate dashboard",
            [
